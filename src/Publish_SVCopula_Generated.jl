...
#=
JLD2.jldsave(
    join(("output/", _SVsetting, " - FrankUnconstrained_SimulationBenchmark.jld2"));
    model=model,
    data=data,
    copula = copula
    errors = errorterms
...
=#
################################################################################
# Set data as Frank Unconstrained data and estimate via other Copulas

_subfolder = "/output/Simulation_Benchmark/"
f_model   =   jldopen(string(pwd(), _subfolder,
...
data = read(f_model, "data")
errorterms = read(f_model, "errors")
