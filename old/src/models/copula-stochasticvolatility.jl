...
#    ℓν = Param(4.0,  truncated( Normal(0.0, 10.0^3), -10.0, 25.)),
    # SDE
    μₛ = Param(0.1, truncated(Normal(10.0, 10.0^4), -1.0, 1.0)),
    ...
    κ = Param(1.00, truncated(Normal(10.0, 10.0^4), 0.0, 100.0)), #10
    #!NOTE Noise term > 0 for both S (in sqrt(σ)) and X (in σ)
    σ = Param(0.10, truncated(Normal(0.5, 10.0^4), 0.01, 2.0)), #1.0
# Standard Parametric Copulas
#    copula = (; ρ = Param(_rho, truncated(Normal(0.0, 10^5), -1.0, 1.0)) ),
    copula = (;
...
function SMCweight(_rng::Random.AbstractRNG, algorithm, objective::Objective{<:ModelWrapper{M}}, proposaltune::P, cumweightsₜ₋₁) where {M<:StochasticVolatilityCopula, P<:ProposalTune}
    ...
    lp = objective.temperature * log_prior(objective.tagged.info.constraint, ModelWrappers.subset(objective.model.val, objective.tagged.parameter) )
